...
const indicatorEdit = ({ indicators, indicator, objectives, responsible }) => {
    ...
    const onSubmit = async (data) => {
        const res = await putRequest({ "src": "indicator", "pup": data, "id": indicator.id })
...
export async function getServerSideProps(context) {
    ...
    const indicators = await fetch(`${SERVER_HOST}/enterprise/${ENTERPRISE_ID}/indicator`)
        .then(res => res.json())
    const indicator = await fetch(`${SERVER_HOST}/enterprise/${ENTERPRISE_ID}/indicator/?id=${params.id}`)
        .then(res => res.json())
